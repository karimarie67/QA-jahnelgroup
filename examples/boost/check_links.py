...
STAGING_DOMAINS = [
    "https://www.stage.boost.cppalliance.org",
    "https://www.stage.boost.org",
    "https://stage.boost.cppalliance.org",
...
def normalize_url(url):
    """Normalize staging URLs to a canonical form."""
    for domain in STAGING_DOMAINS:
        if url.startswith(domain):
            return url.replace(domain, "https://www.stage.boost.org")
    return url

def is_staging_url(url):
    """Check if URL is on any staging domain."""
    return any(url.startswith(domain) for domain in STAGING_DOMAINS)
...
async def main():
    print(f"Starting link check at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    async with async_playwright() as p:
        ...
        start_urls = [
            "https://www.stage.boost.cppalliance.org/",
            "https://www.stage.boost.cppalliance.org/libraries/",
            "https://www.stage.boost.cppalliance.org/docs/",
            "https://www.stage.boost.cppalliance.org/releases/",
            "https://www.stage.boost.cppalliance.org/news/",
            "https://www.stage.boost.cppalliance.org/community/",
        ]
        ...
        for version in VERSIONS_TO_CHECK:
            version_url = f"https://www.stage.boost.org/doc/libs/{version}/"
            ...
            for lib in LIBRARIES_TO_SPOT_CHECK:
                lib_url = f"https://www.stage.boost.org/doc/libs/{version}/libs/{lib}/"
    ...
    # Save reports
    if broken:
        filename = f'broken-links-{timestamp}.csv'
    ...
    if malformed_paths:
        filename = f'malformed-paths-{timestamp}.csv'
        with open(filename, 'w', newline='', encoding='utf-8') as f:
